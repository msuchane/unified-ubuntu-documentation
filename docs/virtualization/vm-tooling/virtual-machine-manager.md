...
There are many more arguments that can be found in the {manpage}`virt-install(1)` manual page. However, here is an explanation of arguments used in the example above, one by one:
