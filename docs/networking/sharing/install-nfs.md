...
The {manpage}`nfs.systemd(7)` manual page has more details on the several systemd units available with the NFS packages.
...
The security options are explained in the {manpage}`exports(5)` manual page, but generally they are:
