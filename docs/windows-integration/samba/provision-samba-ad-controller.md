...
With that information at hand and verified, we can perform further checks. Let's get information about the DNS service provided by this domain controller (the output here is quite long and has been truncated):
...
The default password policy is quite severe, requiring complex passwords. To display the current policy (which shows the default password policy for the domain), run:
